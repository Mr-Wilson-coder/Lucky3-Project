...
const Home = () => {
  ...
  const columns = [
    ...
    { 
      key: 'multiplier', 
      header: 'MULTIPLIER',
      render: (value) => <span className="font-bold text-orange-600">{value}</span>
    },
    { 
      key: 'payout', 
            ...
            <div className="flex-1 max-w-md">
              <GameCard className="bg-white/10 backdrop-blur-md border-white/20 shadow-2xl">
                <div className="text-center mb-6">
                  <h3 className="text-xl font-bold mb-2">DRAW EVERY 30 MINUTES</h3>
                  <input
                    type="tel"
                    placeholder="+254 Mobile Phone Number"
                    value={phoneNumber}
                    onChange={(e) => setPhoneNumber(e.target.value)}
                    className="w-full mb-4 px-3 py-2 bg-white/20 border border-white/30 rounded-md text-white placeholder:text-white/70 focus:outline-none focus:ring-2 focus:ring-white/50"
                  />
                </div>

                <div className="mb-6">
                  <h4 className="text-center font-semibold mb-4">WHAT ARE YOUR 3 LUCKY NUMBERS?</h4>
                  ...
                  <div className="text-center mb-4">
                    <p className="mb-2 font-semibold">YOUR STAKE</p>
                    <input
                      type="text"
                      value={`KSH ${stakeAmount} Min`}
                      onChange={(e) => setStakeAmount(e.target.value.replace(/[^\d]/g, ''))}
                      className="w-full text-center px-3 py-2 bg-white/20 border border-white/30 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-white/50"
                    />
                  </div>
        ...
        <div className="container mx-auto px-4">
          <GameCard className="bg-gradient-to-br from-red-500 to-red-600 border-0 shadow-2xl">
            <div className="flex items-center justify-between text-white">
              ...
              <div className="bg-white rounded-full w-32 h-32 flex items-center justify-center shadow-lg ml-8">
          ...
          <div className="mt-12 bg-white rounded-xl shadow-lg p-8">
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
              <div className="flex items-start space-x-4">
                ...
                <p className="text-gray-700">Go to M-PESA on your phone and select Lipa na M-PESA. Enter Mpesa paybill number 150150</p>
              </div>
              <div className="flex items-start space-x-4">
                <span className="bg-indigo-500 text-white rounded-full w-8 h-8 flex items-center justify-center font-bold">2</span>
                <p className="text-gray-700">Enter your 3 lucky numbers from 0-9 followed by WEB on the account name section eg 637 WEB</p>
              </div>
              <div className="flex items-start space-x-4">
                <span className="bg-indigo-500 text-white rounded-full w-8 h-8 flex items-center justify-center font-bold">3</span>
                <p className="text-gray-700">Enter your bet amount. Minimum 10 KES maximum 1,000 KES</p>
              </div>
            </div>

            <div className="text-center mt-8">
              <p className="text-lg font-semibold text-gray-700 mb-4">That's it! You will receive your ticket by SMS.</p>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-800 mb-4">Odds & Payouts - Example Table</h2>
            <p className="text-gray-600">Every bet is split into a "hybrid bet" - 50% is bet on matching 2 numbers and 50% is bet on matching 3 numbers.</p>
          </div>

          <GameCard className="shadow-xl">
            <ResultsTable data={oddsData} columns={columns} />
          </GameCard>
        </div>
      </section>
    </div>
